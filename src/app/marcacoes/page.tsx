...
export default function MarcacoesPage() {
      ...
      <>
        ...
        <main className="min-h-screen bg-ink flex items-center justify-center px-4 pt-16">
          <div className="text-center max-w-md">
            <div className="w-20 h-20 border border-gold-400/50 flex items-center justify-center mx-auto mb-8">
              <span className="font-heading text-gold-400 text-3xl font-bold">✓</span>
            </div>
            <p className="font-accent italic text-gold-400 text-sm tracking-widest mb-3">
              Pedido recebido
            </p>
            <h1 className="font-heading text-4xl font-bold uppercase tracking-stamp text-cream-200 mb-4">
              Marcação Confirmada
            </h1>
            <div className="flex items-center gap-3 justify-center mb-8">
              <div className="h-px flex-1 bg-gold-600/40" />
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                ...
                className="bg-gold-400 hover:bg-gold-300 text-ink font-heading font-bold px-7 py-3 tracking-stamp uppercase text-sm transition-colors"
              >
                Nova Marcação
              </button>
              <Link
                href="/"
                className="border border-gold-600/50 text-gold-400 hover:bg-gold-400/10 font-heading font-medium px-7 py-3 tracking-stamp uppercase text-sm text-center transition-colors"
              >
                Voltar ao Início
              </Link>
            </div>
  ...
  const labelBase = "block font-heading text-[10px] uppercase tracking-stamp text-cream-400 mb-2";
      ...
      <main className="min-h-screen bg-ink pt-24 pb-16 px-4">
        <div className="max-w-xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h1 className="font-heading text-5xl font-bold uppercase tracking-stamp text-cream-200 mb-4">
              Marcação Online
            </h1>
          ...
          <form
            ...
            className="bg-dark-700 border border-dark-500 border-vintage p-8 sm:p-10 space-y-6"
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  value={form.data}
                  onChange={handleChange}
                  className={`${inputBase} ${isSunday(form.data) ? "border-red-500/70" : ""}`}
                />
                {isSunday(form.data) && (
                  <p className="font-sans text-red-400 text-xs mt-1.5">Fechado aos domingos.</p>
                )}
              </div>
              <div>
            ...
            <button
              ...
              className="w-full bg-gold-400 hover:bg-gold-300 disabled:opacity-50 disabled:cursor-not-allowed text-ink font-heading font-bold py-4 tracking-stamp uppercase text-sm transition-all duration-200"
            >
